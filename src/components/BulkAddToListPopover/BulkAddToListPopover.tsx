...
export const BulkAddToListPopover: React.FC<BulkAddProps> = ({ movies, onClose }) => {
  ...
  const handleAdd = (listId: string) => {
    // Формуємо масив об'єктів, які відповідають інтерфейсу WatchlistMovie
    const moviesToAdd = movies.map(m => ({
      ...
      genres: [m.genre], 
      rating: m.rating,
      posterUrl: m.posterUrl || "", // Переконуємось, що це string, а не undefined
      addedAt: Date.now(),
      runtime: 0 // <--- ДОДАЛИ ЗАГЛУШКУ, щоб TypeScript не сварився
    }));
      ...
      <div className="max-h-60 overflow-y-auto custom-scrollbar p-1">
        {userLists.map(list => (
          ...
          >
            ...
            <span className="text-[10px] text-gray-400">{list.movies.length}</span>
          </button>
        ))}
      </div>
    </div>
